...
Other C++ standards are documented and publicly accessible on the Chaste wiki.
...
The code for each example is available to download as supplementary material and can also be found at \protect\url{https://chaste.cs.ox.ac.uk/cgi-bin/trac.cgi/wiki/PaperTutorials/Plos2012}.
...
(ii) Vertex dynamics models (2D) represent cells as polygons (or polyhedrons) whose vertices move in response to forces. 
...
In low oxygen conditions cells cease to proliferate, and in very low conditions they die, leaving a void in the centre of the spheroid (see Figure~\ref{fig:TumourSpheroid} \highlight{and video S1}), as observed in experiment.
...
Over the course of the simulation each of the four crypts becomes monoclonal, leading to a villus carrying each of the four clonal populations  (\highlight{see video S2}).
...
In Figure~\ref{fig:SpiralWave} we present a snapshot of a two-dimensional monodomain simulation which reproduces one of the results of a study into electrical wave dynamics \cite{Qu2000Origins} (\highlight{see video S3}).
A stable re-entrant spiral wave is generated on a 3cm x 3cm mesh, by appropriate stimulation and alteration of ion-channel expression in a cardiac action potential model \cite{Luo1991Model}.
...
Figure~\ref{fig:TwistyWedge} shows varying fibre directions throughout a three-dimensional block of tissue, alongside the spread of an electrical wave, and subsequent deformation (\highlight{see also video S4}).
...
The authors have used Chaste to integrate the detailed cellular electrophysiology of the stomach into a whole organ framework with a view to studying the effects of genetic mutations as well as diseases such as diabetes \cite{corrias2012modelling} \highlight{Alberto: is book chapter available to cite?}.
...
Releases 1.0 -- 3.0 were under the LGPL v2.0 licence, v3.1 and future releases are under the more flexible 3-clause BSD licence, to facilitate use of the code by industrial partners.
...
\begin{itemize}
    ...
    \item A Chaste `project' download.
    \item Video S1.
    \item Video S2.
    \item Video S3.
    \item Video S4.
