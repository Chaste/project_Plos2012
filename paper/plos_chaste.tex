...
The code provides modules for handling common components of computational models, such as meshes and ODE/PDE solvers, thereby avoiding the need for researcher to `re-invent the wheel' with each new project, accelerating the rate of progress in new applications.
...
It is at the forefront of the cardiac and cell-based modelling fields, being one of the only open software platforms available for these applications.
...
Pair programming has additional benefits: there is no one person who takes sole responsibility for any part of the code, this is especially useful in an academic setting as people move on to new projects frequently.
...
When bugs do occur, it is typically because functionality is expected that hasn't ever been tested, and the first step in the solution is to write more tests.
...
Cellular Potts models are also available (1--3D), in which a cell occupies a number of lattice sites \highlight{[refs including CompuCell3d]}, and additional sites are included or removed to minimize an energy function.
...
(ii) cell-vertex models (2D, 3D planned in future). Cells are defined by the vertices of their polygonal perimeter, these move according to energy minimization formulae of various types - \highlight{list Nagai-Honda, Weiliky Oster etc. here perhaps a sentence on each, include a generic energy minimization equation here?, cite ...}.
...
We support `user projects' for individual studies using Chaste, this allows storage of code in the version-control repository, and provides an interface to the testing framework.
If common code occurs in more than one user project, then generally it is moved to the central Chaste repository, becoming publicly available.
...
Other groups are starting to use Chaste for a large variety of simulations, including: lattice-based multiple occupancy cellular automata (Nottingham \highlight{cite Holga's stuff?});  the effects of radiation on tissue (NASA); sexually transmitted infection modelling (Nottingham); drug-induced changes to cardiac rhythm (FDA); brain simulations (\highlight{ref for these}).
