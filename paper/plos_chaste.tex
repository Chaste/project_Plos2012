...
We support `user projects' for individuals using Chaste to store their code in the version-control repository, and to utilise the testing framework.
If common code occurs in more than one user project, then it is suggested that it be moved to the central Chaste repository.
...
\begin{enumerate}
    ...
    \item Independent benchmarking of the code - scales well to 2048 cores \cite{Strazdins2011}.
    \item Some of PreDiCT stuff (close to real time) - because of parallelism PETSc \cite{petsc} and metis \cite{karypis1998fast}.
    \item Being used to test new numerical schemes - adaptive meshing \cite{Southern2012Parallel}.
...
In the near future, we are planning to add support for the following types of simulation: Navier-Stokes fluid flow; parallel electro-mechanics; Purkinje-myocyte coupling; lung mechanics; import of SBML models for signalling pathways and cell-cycle models.
...
\comment{ask for permission to say each of these people are working on these...}
Other groups are starting to use Chaste for a large variety of simulations, including: lattice-based multiple occupancy cellular automata (Nottingham \comment{cite Holga's stuff?});  the effects of radiation on tissue (NASA); sexually transmitted infection modelling (Nottingham); drug-induced changes to cardiac rhythm (FDA); brain simulations (\comment{}).
