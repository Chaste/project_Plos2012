...
Rafel Bordas$^{1}$, Jonathan Cooper$^{1}$, Alberto Corrias$^3$, Yohan Davit$^4$, 
Sara-Jane Dunn$^5$, Alexander G. Fletcher$^6$, Daniel G. Harvey$^{1}$, 
James M. Osborne$^{1}$, Pras Pathmanathan$^{1,7}$, Joe Pitt-Francis$^{1}$, 
...
The code provides modules for handling common scientific computing components, such as meshes and solvers of ordinary and partial differential equations (ODEs/PDEs).
...
Chaste is now regularly used on supercomputing clusters to simulate the activity of the human heart, and investigate cell behaviour in the development of cancers. 
The Chaste structure admits quick extensions for new projects, and we believe there are many  components that could be used, and contributed to, by the wider scientific community. 
...
To date, the applications our groups have focussed on have been in computational physiology and biophysics.
...
In this article we describe the capabilities of version \highlight{3.1} through the use of examples, novel scientific applications that have been enabled, and future directions. 
It is our intention that all of the examples in this article can be easily reproduced on a desktop PC, they are therefore less computationally intensive than many of the simulations performed in active research.
...
Chaste is written in C++, a fairly low-level compiled language that allows object-oriented class definitions.
...
Pair programming has additional benefits in that no single person takes sole responsibility for any part of the code.
...
These standards are documented and publicly accessible on the Chaste wiki.
...
The code for each example is available to download as supplementary material and can also be found at \protect\url{http://chaste.here}. \highlight{Fix link.}
For examples of state-of-the-art simulation results obtained using Chaste, please refer to the papers cited herein.
...
\textbf{On-lattice} models which are supported include cellular automata (1--3D) \cite{moreira2002cellular}, in which one lattice site is associated with one or more cells.
...
In Figure~\ref{fig:TumourSpheroid} we show a cut-away view of a tumour spheroid.
...
In low oxygen conditions cells die, leaving a void in the centre of the spheroid, as observed in experiment (\highlight{see Supplementary Material video 1}).
...
Over the course of the simulation each of the four crypts becomes monoclonal, leading to a villus carrying each of the four clonal populations  (\highlight{see Supplementary Material video 2}).
...
This leads to a dynamic pattern of Delta-Notch with high/low activity in neighbouring cells, thought to lead to differentiation into secretory and absorbative cells.
...
In Figure~\ref{fig:SpiralWave} we present a snapshot of a two-dimensional monodomain simulation which reproduces one of the results of a study into electrical wave dynamics \cite{Qu2000Origins} (\highlight{see Supplementary Material video 3}).
A stable re-entrant spiral wave is generated on a 3x3 centimetre mesh, by appropriate stimulation and alteration of ion-channel expression in a cardiac action potential model \cite{Luo1991Model}.
The behaviour of electrical waves is commonly studied in cardiac electrophysiology to understand how changes to ion-channel dynamics, through disease or drug block, can lead to the onset of fatal arrhythmias.

Recently we have also begun to work on electromechanics \cite{Pathmanathan2010}, this becoming publicly available in v3.0. 
We use this electromechanics code for our final example in Figure~\ref{fig:TwistyWedge}.
Figure~\ref{fig:TwistyWedge} shows varying fibre directions throughout a three-dimensional block of tissue, alongside the spread of an electrical wave, and subsequent deformation.
Deformation occurs due to cellular tension development in response to electrical activation and calcium release, coupled to a model of mechanical properties of cardiac tissue (\highlight{see Supplementary Material video 4}).

FIGURE~\ref{fig:TwistyWedge} HERE.

A mesh of the human heart embedded within a torso has enabled us to simulate the 12-lead human body surface ECG, and to use this to predict ECG changes under drug action \cite{zemzemi2012}. 
...
Simulations have also been performed to study defibrillation of a human heart in arrhythmia \highlight{[Ey Miguel: ref?]}, with a view to improving medical devices and interventions.

Dynamic loading and units conversion of CellML files \cite{Cooper2011cuc} has enabled studies that consider the extensive variability between different models \cite{Pathmanathan2011,Mirams2011,Cooper2011}.
Our group has also used Chaste to study the effect of stochasticity in cardiac models \cite{Walmsley2012,Dutta2011,Pueyo2011}, a potential cause of variability in experimental recordings.

Improvements to the numerical algorithms used in cardiac simulation have been a large focus of our research efforts, and have enabled the novel biological problems above to be studied. We have focussed on matrix preconditioners \cite{Bernabeu2009a,Bernabeu2010}, adaptivity \cite{Southern2012Parallel}, and choice of finite element formulation \cite{Pathmanathan2010ngb,Pathmanathan2011,arthurs2012efficient}.
...
lattice-based multiple occupancy cellular automata (Markus Owen, Nottingham);  % approved
the effects of radiation on tissue (Shaowen Hu, NASA); % approved
sexually transmitted infection modelling (Martin Nelson, Nottingham); % approved
cardiac electrophysiological modelling (FDA); % alright Pras?!?
drug-induced changes to cardiac rhythm (Safety Pharmacology, GlaxoSmithKline); % approved
neuron electrophysiology (Andres Agudelo-Toro, G\"{o}ttingen). % on our website so fine I guess
...
\item 2020 Science: EPSRC and Microsoft Research, Cambridge through grant EP/I017909/1 (\url{www.2020science.net}).
...
\highlight{Gary: todo, make sure there is a movie of each of these available as supplementary material...}
...
Cell centres, nodes of a mesh, are represented by spherical shells and coloured according to oxygen concentration. 
Proliferation is dependent on oxygen, which diffuses and is taken up by cells in the spheroid, so only cells near the outer rim divide.
Under low oxygen conditions cell death occurs, leaving a necrotic core at the spheroid centre.
...
Left: cells are labelled according to their ancestor cell, each crypt gives rise to a monoclonal population, the villus is comprised of a population from each crypt. 
Right: the same simulation, here cells are labelled according to Delta levels (nondimensionalised), Delta-Notch patterning occurs due to a signalling model inside each cell, which depends on the activity of neighbouring cells, this patterning is thought to lead to differentiation into secretory and absorbative cell types.
}
...
This figure displays the membrane voltage in a two-dimensional 3cm $\times$ 3cm monodomain simulation using the Luo-Rudy 1991 action-potential model \cite{Luo1991Model} with the modifications and protocol suggested in \cite{Qu2000Origins}.
}
...
\end{center}
\caption{
{\bf Cardiac electromechanics in a ventricular wedge:} left, fibre orientation; middle and right, simulation of electrical propagation and deformation (middle: 1ms after stimulus on left face; right: 35ms after stimulus).
\highlight{To do: voltage colourbar or description in text.}
}
...
Instructions can be found at \url{https://chaste.cs.ox.ac.uk/cgi-bin/trac.cgi/wiki/InstallGuides}.
...
\begin{table}[!ht]
    \caption{
    \bf{Chaste Dependencies.} For the full text of all licences examine the contents of \protect\url{docs/licences.html} 
...
MPICH &  \url{http://www.mcs.anl.gov/research/projects/mpich2} & Custom\\
