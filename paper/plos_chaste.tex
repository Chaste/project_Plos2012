...
Here we briefly describe the components of the code base and subsequent capabilities.
We will discuss new scientific insight Chaste has enabled in the Results section.

\begin{itemize}
 \item \texttt{global} --- basic code for Chaste warnings and exceptions, mathematics (including a random number generator), time stepping classes, code for checkpointing (saving and loading, which utilises the boost serialization library), and wrappers for libraries such as PETSc.
...
In terms of cellular mechanics, there are two main types of model: on- and off-lattice. 
...
Cellular Potts models are also available (1--3D), in which a cell occupies a number of lattice sites \highlight{[refs]}, and additional sites are included or removed to minimize an energy function.
...
We support `user projects' for individuals using Chaste, this allows storage of code in the version-control repository, and provides an interface to the testing framework.
...
Tumour Spheroids (oxygen PDE)

Delta-Notch patterning --- see Figure~\ref{fig:DeltaNotchFootball}

\subsection*{Heart}

Chaste is one of the fastest, most accurate cardiac electrophysiology solvers, and is the only large-scale open source solver available \cite{bordas2009simulation,niederer2011verification}.
...
\garycomment{ask for permission to say each of these people are working on these...}
Other groups are starting to use Chaste for a large variety of simulations, including: lattice-based multiple occupancy cellular automata (Nottingham \garycomment{cite Holga's stuff?});  the effects of radiation on tissue (NASA); sexually transmitted infection modelling (Nottingham); drug-induced changes to cardiac rhythm (FDA); brain simulations (\garycomment{}).
...
\begin{figure}[!ht]
\begin{center}
%\includegraphics[width=4in]{multi_crypts.eps}
\end{center}
\caption{
...
\begin{figure}[!ht]
\begin{center}
%\includegraphics[width=4in]{delta_notch.eps}
\end{center}
\caption{
...
\begin{figure}[!ht]
\begin{center}
%\includegraphics[width=4in]{spiral_wave.eps}
\end{center}
\caption{
...
\begin{figure}[!ht]
\begin{center}
%\includegraphics[width=4in]{twisty_wedge.eps}
\end{center}
\caption{
