...
Code development has been driven by two main applications. (1) In cardiac electrophysiology a large number of studies have been enabled and performed, including high-performance computing investigations of defibrillation in realistic human cardiac geometry. (2) New models for the initiation and growth of cancer have been developed; individual-cell-based simulations have provided novel insight into the role of stem cells in the colorectal crypt.
Chaste is now being applied to a far wider range of problems. 
The code provides modules for handling common components of computational models, such as meshes and ODE/PDE solvers, thereby avoiding the need for researchers to `re-invent the wheel' with each new project, accelerating the rate of progress in new applications.
...
It is still common for each new PhD student or postdoctoral researcher to `re-invent the wheel' and develop, for example, their own mesh structures, ordinary/partial differential equation (ODE/PDE) solvers and input/output (IO) interfaces. 
...
\highlight{Short tribute to other open source efforts: OpenCMISS \cite{Bradley2011}, CompuCell3D \cite{Cickovski2007}.}
...
Examples of problems falling into these categories include cardiac electrophysiology and electromechanics, tumour growth, and developmental biology \highlight{refs?}.
...
Chaste is built with C++, which is a reasonably low-level compiled language that allows object-oriented class definitions.
...
Chaste is developed using a so-called `agile' approach. \highlight{ref?}
...
In an academic setting we have found that this need not be insisted upon, but we try to use it wherever possible.
Pair programming has additional benefits. For example, no single person takes sole responsibility for any part of the code; this is especially useful in an academic setting as people move on to new projects on a frequent basis.

To maintain bug-free code that retains its function in this rapid re-writing style of development, we believe that scientific agile programming must also be `test-driven'.

\subsubsection*{Test-driven development}

Test-driven development \highlight{ref?} is fundamental to our approach, and is the opposite of what computational scientists typically do. 
...
All test code is committed to the central repository at the same time as the source code that it tests.
Upon each commit, all of the test code is run in order to check that no function has been inadvertently broken.
...
This involves code for ODEs, PDEs, meshes, i/o, visualization, etc, which is required by many different application areas in mathematical biology, along with specialised modules for particular application areas.
...
\begin{itemize}
 \item \texttt{global} --- mathematics (including a random number generator), time stepping classes, code for checkpointing (saving and loading, which utilises the boost serialization library), wrappers for libraries such as PETSc, and code to handle warnings and exceptions.
 \item \texttt{io} (input/output) --- contains code for reading, writing and conversion of various file formats, together with modules to handle the HDF5 scientific file format \highlight{[ref]}, which enables distributed data to be collated and stored in a single file.
 \item \texttt{mesh} --- contains code for storing linear/quadratic tetrahedral meshes and vertex meshes in memory; nodes, elements, boundary properties; mesh reading; generation; writers for triangle, meshalyzer, cmgui and VTK (paraview) formats.
 ...
 \item \texttt{continuum\_mechanics} --- code for compressible and incompressible linear and nonlinear elasticity. \highlight{Pras again}.
\end{itemize}
...
On-lattice models which are supported include cellular automata (1--3D) \highlight{[ref]}, in which one lattice site is associated with one or more cells. Cellular automata have been widely used for \highlight{... cite some of Nottingham stuff?}.
Cellular Potts models are also available (1--3D), in which a cell occupies a number of lattice sites \highlight{[refs including CompuCell3d]}\cite{Graner1992}, and additional sites are included or removed to minimize an energy function.
...
node-based (1--3D) --- neighbours are any nodes within a certain interaction distance \highlight{e.g. [refs], include force equation sum over neighbours here?};
mesh-based (1--3D) --- neighbours are nodes which share elements of a mesh (defined by a Delaunay triangulation of the cell centres using Triangle (2D) \cite{shewchuk96b} or Tetgen (3D) \cite{si2005}.) \highlight{e.g. [refs]}.
...
It is standard to make the simplifying assumption that the motion of vertices is overdamped \cite{Drasdo2000}, and inertial terms are small compared to dissipative terms.
...
The \texttt{heart} component provides fast and accurate solution  of electromechanics problems on large meshes, optimized for high-performance computing facilities by using PETSc for parallel linear algebra \cite{petsc}, and METIS for mesh distribution \cite{karypis1998fast}.
Simulations can be performed based upon single cell ODE systems using CVODE \cite{hindmarsh2005sundials}, or tissue PDE systems including monodomain\highlight{[ref]}, bidomain\highlight{[ref]}, bidomain-with-bath\highlight{[ref]}, and our new extended-bidomain system \cite{corrias2012modelling}.
...
Post-processing can be performed to obtain quantities such as action potential durations, pseudo-ECGs \highlight{[ref]}, and conduction velocities.
...
We have extensively used this component with off-lattice mesh-based models, \cite{VanLeeuwen2009,fletcher2012mathematical,Dunn2012,Dunn2012Plos}.
\highlight{Insights on monoclonality in \cite{VanLeeuwen2009}, and how they have been experimentally proven since...}

Off-lattice nearest-neighbour-defined interactions, springs, overlapping spheres, pretty versatile --- Figure~\ref{fig:FourCrypts}.

We have studied the suitability of above models and any `quirks' they introduce \cite{Pathmanathan2009,Osborne2010}.

Tumour spheroids (oxygen PDE) --- see Figure~\ref{fig:TumourSpheroid} \highlight{Can we get one of these in 3D with a cut-away view?}.

Delta-Notch patterning --- see Figure~\ref{fig:FourCrypts}.

\subsection*{Heart}

Chaste is one of the fastest cardiac electrophysiology solvers, and is the only large-scale open source solver available \cite{bordas2009simulation,niederer2011verification}.
Independent benchmarking has shown \highlight{number here} scaling to 2048 cores \cite{Strazdins2011}.

In Figure~\ref{fig:TwistyWedge} we present a snapshot of a spiral wave simulation, commonly studied in cardiac electrophysiology to learn about the electrical dynamics leading to the initiation of arrhythmic behaviour. \highlight{This example should run in xxx seconds on a xx machine?}

Improvements to the numerical methods have been a large focus of our research efforts \highlight{expand on these} \cite{Bernabeu2009a,Pathmanathan2010ngb,Bernabeu2010,Pathmanathan2011,Southern2012Parallel} \highlight{cite Megan, and include as co-author?}.
...
Simulation have been performed to study defibrillation of a human heart in arrhythmia \highlight{[Miguel ref?]}, with a view to improving medical devices and interventions.

An automatic conversion of cardiac cell models, defined by CellML files, has been developed \cite{Cooper2011cuc}.
...
Our group has used Chaste to study the effect of stochasticity in action potential models \cite{Walmsley2012}. 
\highlight{ and \cite{Dutta2011}}.

Recently we have begun to work on electro-mechanics, and the latest code release has made this available. In Figure~\ref{fig:TwistyWedge} we show the spread of an electrical wave in a slab of cardiac tissue, and its subsequent deformation due to an electrically-dependent contraction model, and tissue mechanics \highlight{Pras sentence here}. 
Feedback from deformation to electrical activity also occurs through the presence of stretch-activated ion channels in the electrophysiology cell model.
...
In the near future, we are planning to add support for the following types of simulation: Navier-Stokes fluid flow; parallel cardiac electro-mechanics; Purkinje-myocyte coupling in cardiac electrophysiology; lung mechanics; import of SBML models for signalling pathways and cell-cycle models.
...
Other groups are starting to use Chaste for a large variety of simulations, including: lattice-based multiple occupancy cellular automata (Nottingham \highlight{cite Holger's stuff?}\cite{Perfahl2011});  the effects of radiation on tissue (NASA); sexually transmitted infection modelling (Nottingham); drug-induced changes to cardiac rhythm (FDA); brain simulations (\highlight{ref for these}).

\subsection*{How to become an active developer}

Anyone can generate a login for the Chaste wiki at\\
...
\begin{figure}[!ht]
\begin{center}
%\includegraphics[width=4in]{images/spheroid.eps}
\end{center}
\caption{
{\bf 3D tumour spheroid?}  
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=4in]{images/multi_crypt.eps}
\end{center}
\caption{
{\bf Four-crypts and villus simulation.}  
The code for this is available to download as supplementary material and can also be found at \texttt{http://chaste.here}. \garycomment{Possibly combine the delta-notch and 4 crypts and a villus simulation? Would get nice pattern of secretory / absorptive cells presumably, which they reckon is delta-notch --- SCIENCE!}
}
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=4in]{images/spiral_wave.eps}
\end{center}
\caption{
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=0.45\linewidth]{images/undeformed_cube.eps}
\includegraphics[width=0.45\linewidth]{images/deformed_cube.eps}
\end{center}
\caption{
