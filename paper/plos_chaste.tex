...
In this article we provide examples that illustrate the types of problems Chaste can be used to solve, which can easily be run on a desktop computer.
...
\begin{itemize}
 \item \texttt{global} --- contains code for basic mathematics (including a random number generator), time stepping, checkpointing (saving and loading simulations, which utilises the Boost serialization library), parallel vector classes, and code to handle warnings and errors.
 ...
 \item \texttt{mesh} --- code for storing linear or quadratic tetrahedral meshes and vertex meshes in memory; nodes, elements, boundary properties; mesh generation; readers and writers for Triangle (2D) \cite{shewchuk96b} or Tetgen (3D) \cite{si2005}, meshalyzer, cmgui and VTK (Paraview) formats.
...
\comment{RB: I think this section needs a few sentences or a paragraph on electromechanics}
...
\highlight{Gary: In the MultipleCrypts code we seem to set the ``Mechanics Cut off length'' twice, once via the population and once via the force law. Is this necessary - if so why?! - if not can we remove one of the calls?}
...
\highlight{Currently \cite{Pathmanathan2010} isn't cited anywhere - include this?}
...
cardiac electrophysiological modelling (FDA); % to ask for approval
drug-induced changes to cardiac rhythm (Safety Pharmacology, GlaxoSmithKline); % approved
brain simulations (\highlight{ref for these?}). % to ask for approval
...
Cell centres, nodes of a mesh, are coloured according to oxygen concentration. 
Proliferation is dependent on the diffusion and uptake of oxygen within the spheroid, so only cells near the rim of the spheroid divide.
Cell death occurs under low oxygen conditions, leaving a necrotic core at the spheroid centre.
...
Right: cells are labelled according to Delta levels (nondimensionalised), Delta-Notch patterning occurs due to a signalling model inside each cell, which depends on the activity of neighbouring cells, this patterning is thought to lead to differentiation into secretory and absorbative cell types.
}
...
\highlight{Voltage colourbar or description in text.}
}
