...
#ifndef LUORUDYCELLFACTORY_HPP_
#define LUORUDYCELLFACTORY_HPP_

#include "LuoRudy1991AnnotatedBackwardEuler.hpp"
#include "AbstractCardiacCellFactory.hpp"
#include "MultiStimulus.hpp"
#include "ChasteCuboid.hpp"

class LuoRudyCellFactory : public AbstractCardiacCellFactory<2>
{
private:
    ...
    double mYExtent;
public:
    LuoRudyCellFactory(double xExtent, double yExtent)
    ...
    AbstractCardiacCell* CreateCardiacCellForTissueNode(unsigned nodeIndex)
    {
        ...
        if ( x < x_threshold_for_S1 )
        {
            ...
            {
                p_cell = new CellLuoRudy1991AnnotatedFromCellMLBackwardEuler(mpSolver, mpBothStimulus);
            }
            else
            {
                p_cell = new CellLuoRudy1991AnnotatedFromCellMLBackwardEuler(mpSolver, mpS1Stimulus);
            }
        }
        else if ( (x < x_threshold_for_S2) && (y < y_threshold_for_S2) )
        {
            p_cell = new CellLuoRudy1991AnnotatedFromCellMLBackwardEuler(mpSolver, mpS2Stimulus);
        }
        else
        {
        ...
        p_cell->SetParameter("membrane_slow_inward_current_conductance",0);
        ...
        if (nodeIndex==0)
        {
            ...
            std::cout << "G_si = " << p_cell->GetAnyVariable("membrane_slow_inward_current_conductance") << "\n";
